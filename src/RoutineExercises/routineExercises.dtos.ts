...
export const createRoutineExerciseSchema = z.object({
    ...
    setsPlanned: z.number().int().min(1, 'El número de series debe ser al menos 1').default(3),
    repsPlanned: z.number().int().min(1, 'El número de repeticiones debe ser al menos 1').default(10),
    weightPlanned: z.number().int().min(0, 'El peso no puede ser negativo').max(400, 'El peso no puede exceder los 400 kg').default(0) // in kg
...
export const updateRoutineExerciseSchema = z.object({
    ...
    setsPlanned: z.number().int().min(1, 'El número de series debe ser al menos 1').default(3).optional(),
    repsPlanned: z.number().int().min(1, 'El número de repeticiones debe ser al menos 1').default(10).optional(),
    weightPlanned: z.number().int().min(0, 'El peso no puede ser negativo').max(400, 'El peso no puede exceder los 400 kg').default(0).optional() // in kg
});
